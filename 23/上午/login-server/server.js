...
// 登录请求
app.get("/meituan/api/login", (req, res) => {
  // 获得客户端的code
  let { code } = req.query;
  if (!code) {
    res.json({
...
// 检查用户是否登录过
app.get('/meituan/api/check_login', (req, res)=>{
...
// 保存用户信息
app.post('/meituan/api/save_userinfo', (req, res)=>{
  //判断用户是否登录过
  if(req.session.userInfo){
    let body = req.body;
    // 保存用户信息
...
// 获得用户信息
app.get('/meituan/api/get_userinfo', (req, res)=>{
  if(req.session.userInfo){
